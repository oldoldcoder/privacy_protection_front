<template>
  <div class="container">
    <h1 class="container-title">包含关系计算</h1>
    <div class="container-tip">
      用户1拥有数据x，用户2拥有范围[y1,y2]，两个用户协同计算，判断x是否在[y1,y2]的范围内，最终结果仅为用户1所知。0代表x在范围[y1,y2]内；1代表x不在范围[y1,y2]内。
    </div>
...
const customRequest = ({ file, onSuccess, onError }) => {
  ...
  axios
    .post('http://10.10.55.25:8082/dd/scc/include', formData, { withCredentials: true })
    .then((response) => {
