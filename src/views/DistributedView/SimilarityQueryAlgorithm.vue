<template>
  <div class="container">
    <h1 class="container-title">相似性查询算法</h1>
...
const customRequest = ({ file, onSuccess, onError }) => {
  ...
  axios
    .post('http://10.10.55.25:8082/DD/ssq/initData', formData, { withCredentials: true })
    .then((response) => {
...
const secondRequest = ({ file, onSuccess, onError }) => {
  ...
  loading.value = true
  axios
    .post('http://10.10.55.25:8082/DD/ssq/query', formData, { withCredentials: true })
    .then((response) => {
      if (response.status === 200) {
