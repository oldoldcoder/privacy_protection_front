<template>
  <div class="container">
    <h1 class="container-title">相似性查询算法（非分布式）</h1>
    ...
    <div class="container-file" v-loading="uploadLoading">
      ...
      >
        ...
        <template #tip>
          <div class="el-upload__tip">数据生成时间：{{ fileTime }}秒</div>
        </template>
      </el-upload>
    </div>
  </div>
...
const formula = `令X = \\left\\{ x_i = \\left( x_{i,1}, x_{i,2}, \\dots, x_{i,d} \\right) \\right\\}_{i=1}^n为数据集。令 \\left( q,τ \\right) 为一个相似性查询请求，则相似性查询旨在X中距q的距离不小于τ的数据记录，即R_q= \\left\\{ x_i | d \\left( x_i,q \\right) ≤ τ \\right\\} 。`
...
const customRequest = ({ file, onSuccess, onError }) => {
  ...
  axios
    .post('http://10.10.55.25:8082/ND/ssq/initData', formData, { withCredentials: true })
    .then((response) => {
...
const secondRequest = ({ file, onSuccess, onError }) => {
  ...
  loading.value = true
  axios
    .post('http://10.10.55.25:8082/ND/ssq/query', formData, { withCredentials: true })
    .then((response) => {
      if (response.status === 200) {
