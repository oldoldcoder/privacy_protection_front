<template>
  <div class="container">
    <h1 class="container-title">范围查询算法（非分布式）</h1>
...
const formula = `给定数据集：X = \\left\\{ x_i = \\left( x_{i,1}, x_{i,2}, \\dots, x_{i,d} \\right) \\right\\}_{i=1}^n和相关的查询范围，返回处于范围内的数据信息。`
...
const customRequest = ({ file, onSuccess, onError }) => {
  ...
  axios
    .post('http://10.10.55.25:8082/ND/range_search/initData', formData, { withCredentials: true })
    .then((response) => {
...
const secondRequest = ({ file, onSuccess, onError }) => {
  ...
  loading.value = true
  axios
    .post('http://10.10.55.25:8082/ND/range_search/query', formData, { withCredentials: true })
    .then((response) => {
      if (response.status === 200) {
