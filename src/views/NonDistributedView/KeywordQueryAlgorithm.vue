<template>
  <div class="container">
    <h1 class="container-title">关键字查询算法（非分布式）</h1>
...
const formula = `关键字查询是一种信息检索方法，旨在通过给定的查询关键字w_i，找到所有包含该关键字的文件标识（文件ID）`
...
const customRequest = ({ file, onSuccess, onError }) => {
  ...
  axios
    .post('http://10.10.55.25:8082/ND/skq/initData', formData, { withCredentials: true })
    .then((response) => {
...
const secondRequest = ({ file, onSuccess, onError }) => {
  ...
  loading.value = true
  axios
    .post('http://10.10.55.25:8082/ND/skq/query', formData, { withCredentials: true })
    .then((response) => {
      if (response.status === 200) {
